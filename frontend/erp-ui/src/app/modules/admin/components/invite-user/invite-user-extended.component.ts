...
@Component({
  selector: 'app-invite-user-extended',
  standalone: true,
  ...
  <div class="flex items-start justify-center min-h-[calc(100vh-64px)] pt-4">
    <div class="w-full max-w-lg bg-white shadow-2xl rounded-2xl p-10 border border-gray-200">
      <p class="text-gray-500 text-sm mb-1">Send invitations to HR, Finance, Inventory, IT Operations, Operations, Sales, or Employees.</p>
...
export class InviteUserExtendedComponent implements OnInit {
  private readonly admin = inject(AdminService);
  private readonly hrApi = inject(HrApiService);

  name = '';
  email = '';
  role: 'EMPLOYEE' | 'HR' | 'FINANCE' | 'INVENTORY' | 'IT_OPERATION' | 'OPERATIONS' | 'SALES_ONLY' = 'EMPLOYEE';
  ...
  submit() {
    ...
    this.admin.inviteUser(payload).subscribe({
      next: () => {
        this.hrApi.ensureEmployee({ email: this.email, fullName: this.name, departmentId: this.departmentId || null, salary: this.baseSalary ?? null, workingHours: this.workingHours ?? null }).subscribe({ next: () => {}, error: () => {} });
